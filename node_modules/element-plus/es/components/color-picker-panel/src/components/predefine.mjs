import predefine_vue_vue_type_script_setup_true_lang_default from "./predefine.vue_vue_type_script_setup_true_lang.mjs";
//#region ../../packages/components/color-picker-panel/src/components/predefine.vue
var predefine_default = predefine_vue_vue_type_script_setup_true_lang_default;
//#endregion
export { predefine_default as default };

//# sourceMappingURL=predefine.mjs.map