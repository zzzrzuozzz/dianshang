  ...
  <div v-loading="loading" class="promo-page">
    <el-card shadow="never" class="panel-card">
      <div class="section-header">
        <span class="section-title">时间段设置 - 活动 {{ activityId }}</span>
        <div>
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.isEdit ? '编辑时间段' : '添加时间段'" width="480px" destroy-on-close>
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="dialog.saving" @click="saveTime">保存</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
const dialog = reactive({
  ...
  saving: false,
  isEdit: false,
  form: { id: '', name: '', startH: 8, startM: 0, startS: 0, endH: 12, endM: 0, endS: 0, enabled: true },
})
...
const openDialog = (row) => {
  ...
  if (row) {
    ...
    Object.assign(dialog.form, { id: '', name: '', enabled: true, startH: 8, startM: 0, startS: 0, endH: 12, endM: 0, endS: 0 })
  }
  dialog.visible = true
}
...
const toggleEnable = async (row) => {
  await toggleGroupBuyTime(activityId, row.id, row.enabled)
...
const handleDelete = (row) => {
  ElMessageBox.confirm(`确定删除「${row.name}」吗？`, '提示', { type: 'warning' })
    .then(async () => {
      await deleteGroupBuyTime(activityId, row.id)
