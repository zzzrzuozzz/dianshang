...
CREATE TABLE IF NOT EXISTS sys_role (
  ...
  role_key VARCHAR(50) NOT NULL UNIQUE COMMENT '角色权限标识',
...
CREATE TABLE IF NOT EXISTS sys_menu (
  ...
  sort_num INT DEFAULT 0,
  create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='菜单及权限要素表';
...
CREATE TABLE IF NOT EXISTS sys_invite_code (
  ...
  role_id BIGINT NOT NULL COMMENT '注册后绑定角色',
  used TINYINT NOT NULL DEFAULT 0 COMMENT '0-未使用 1-已使用',
  used_by_admin_id BIGINT DEFAULT NULL COMMENT '使用者管理员ID',
  used_account VARCHAR(100) DEFAULT NULL COMMENT '注册时填写的手机/邮箱',
  used_account_type VARCHAR(10) DEFAULT NULL COMMENT 'PHONE/EMAIL',
  created_by BIGINT DEFAULT NULL COMMENT '生成人',
  remark VARCHAR(255) DEFAULT NULL,
  used_time TIMESTAMP DEFAULT NULL,
  create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS idx_invite_used ON sys_invite_code (used, create_time);

CREATE TABLE IF NOT EXISTS sys_notice (
  id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  title VARCHAR(100) NOT NULL COMMENT '通知标题',
  content CLOB NOT NULL COMMENT '通知正文/富文本',
  notice_type VARCHAR(20) DEFAULT 'SYSTEM' COMMENT 'SYSTEM/AUDIT/ALARM',
  level VARCHAR(10) DEFAULT 'INFO' COMMENT 'INFO/WARNING/DANGER',
  status TINYINT DEFAULT 0 COMMENT '0-未读 1-已读',
  sender VARCHAR(50) DEFAULT '系统通知',
  create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS idx_notice_status_time ON sys_notice (status, create_time);

CREATE TABLE IF NOT EXISTS sys_config (
  id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  config_key VARCHAR(50) NOT NULL UNIQUE COMMENT '配置项键名',
  config_value VARCHAR(255) NOT NULL COMMENT '配置项值',
  remark VARCHAR(100) DEFAULT NULL COMMENT '配置项描述',
  update_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='系统全局参数设置表';
...
ALTER TABLE pms_product ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMP;
...
CREATE TABLE IF NOT EXISTS cms_topic_type (
  id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS cms_topic (
  ...
  images_json CLOB,
  audience_json CLOB,
  specify_products BOOLEAN NOT NULL DEFAULT FALSE,
  product_ids_json CLOB,
  sort_num INT NOT NULL DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS idx_cms_topic_type ON cms_topic (type_id);
CREATE INDEX IF NOT EXISTS idx_cms_topic_status ON cms_topic (status);

CREATE TABLE IF NOT EXISTS cms_topic_comment (
  comment_code VARCHAR(20) NOT NULL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_cms_topic_comment ON cms_topic_comment (topic_code);

CREATE TABLE IF NOT EXISTS cms_help_type (
  id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  type_code VARCHAR(20) NOT NULL UNIQUE,
  name VARCHAR(100) NOT NULL,
  icon VARCHAR(500),
...
CREATE TABLE IF NOT EXISTS cms_help_article (
